...
**Trace content remains byte-count-only for tool calls.** The failure-geometry audit's F16 finding (tool args/results not captured) was not addressed in this window. Tool call traces still record `inputBytes`, `resultBytes`, `isError` — not the content. The `AFK_CAPTURE_SUBAGENT_OUTPUT=1` and `AFK_CAPTURE_SUBAGENT_PROMPTS=1` flags exist for targeted capture but are off by default.
